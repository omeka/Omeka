...
class Omeka_Helper_DisplayRandomFeaturedItemsTest extends Omeka_Test_AppTestCase
{
    public function testDisplayRandomFeaturedItems()
    {
        ...
        $this->assertStringContainsString('<h3><a href="/items/show/'. $ids[0]. '">Title 1</a></h3>', $html);
        $this->assertStringContainsString('<p class="item-description">Description for item 1.</p>', $html);
    }
    ...
    public function testDisplayNoRandomFeaturedItems()
    {
        ...
        $this->dispatch('/');
        $html = random_featured_items();
        $this->assertStringContainsString('<p>No featured items are available.</p>', $html);
    }

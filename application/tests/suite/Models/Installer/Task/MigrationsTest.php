 ...
 * @copyright Roy Rosenzweig Center for History and New Media, 2007-2010
 */
class Installer_Task_MigrationsTest extends Omeka_Test_TestCase
{
    ...
    public function testCreatesMigrationSchema()
    {
        ...
        foreach ($queryProfiles as $profile) {
            if (strstr($profile->getQuery(), "CREATE TABLE IF NOT EXISTS `test_schema_migrations`")) {
                $ranCreate = true;
            } elseif (strstr($profile->getQuery(), "INSERT INTO `test_options`")) {
                $ranOptionInsert = true;
            } elseif (strstr($profile->getQuery(), "INSERT INTO test_schema_migrations")) {

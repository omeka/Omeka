...
class Omeka_Db_Migration_ManagerTest extends Omeka_Test_TestCase
{
    ...
    public function setUpLegacy()
    {
        $this->db = $this->getMockBuilder('Omeka_Db')
            ->setMethods(['fetchCol', 'getAdapter', 'query'])
            ->disableOriginalConstructor()
            ->getMock();
        $this->dbAdapter = $this->getMock('Zend_Db_Adapter_Mysqli', ['insert'], [], '', false);
        ...
        $this->bootstrap->getContainer()->options = [Omeka_Db_Migration_Manager::VERSION_OPTION_NAME => '2.0'];
    ...
    public function testMigratingToCurrentTime()
    {
        $this->db->expects($this->any())
                 ...
                 ->will($this->returnValue(['20080717013528', self::ALREADY_RUN_MIGRATION]));
    ...
    public function testMigratingWithPre20Version()
    {
        $this->db->expects($this->any())
                ->method('fetchCol')
                ->with($this->stringContains('SELECT'))
                ->will($this->returnValue([self::ALREADY_RUN_MIGRATION]));

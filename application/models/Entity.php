...
class Entity extends Omeka_Record
{
    ...
    protected function _validate()
    {
        ...
        if (is_numeric($this->parent_id)) {
            ...
            } else if ($this->Parent->isDescendantOf($this)) {
        ...
        if (($this->type == 'Person') && empty($this->first_name) && empty($this->last_name)) {
            $this->addError('Name', 'The name for a person may not be completely blank.');
        }
        
        // Blank institution name for an 'Institution' is not OK
        if (($this->type == 'Institution') && empty($this->institution)) {
            $this->addError('Name', 'The name of an institution may not be blank.');
        }
    }
    
    /**
     * Trim all the data that comes in via the form, make sure that parent_id is 
     * a valid foreign key.
     *
     * @return array
     **/
    ...
    /**
     * This should be a template method for the sub-classes that extend this 
     * (Individual, Institution)
     *
     * @todo On conversion to new version of doctrine, make this method a 
     * template method
     * @return string
     **/

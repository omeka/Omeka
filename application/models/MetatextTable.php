...
class MetatextTable extends Omeka_Db_Table
{
    ...
    public function findByItem($item_id)
    {
        ...
        $sql = "
        SELECT DISTINCT(mf.name) as name, mt.id, mt.text as text, mt.metafield_id, mf.description, mt.item_id
        FROM $db->Metatext mt 
        INNER JOIN $db->Metafield mf 
        ON mf.id = mt.metafield_id
        WHERE mt.item_id = ?
        GROUP BY name";

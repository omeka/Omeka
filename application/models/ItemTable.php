...
{	
	...
	public function filterBySearch($select, $params)
	{
		...
		if(isset($params['search']) or isset($params['advanced_search'])) {
	...
	{
		$select->joinInner( array('c'=>$this->getDb()->Collection), 'i.collection_id = c.id', array());
	...
	{		
		$select->joinInner( array('ty'=>$this->getDb()->Type),'i.type_id = ty.id', array());
	...
	{		
		$select->joinInner( array('tg'=>$this->getDb()->Taggings),'tg.relation_id = i.id', array());
		$select->joinInner( array('t'=>$this->getDb()->Tag), 'tg.tag_id = t.id', array());
		if(!is_array($tags) )
		{
			$tags = explode(',', $tags);
		}
	...
	public function filterByUserOrEntity($select, $entityId, $isUser=true)
	{
		...
		$select->joinLeft( array('ie'=>"$db->EntitiesRelations"), 'ie.relation_id = i.id', array());
		$select->joinLeft( array('e'=>"$db->Entity"), 'e.id = ie.entity_id', array());
		
		if($isUser) {
			$select->joinLeft( array('u'=>"$db->User"), 'u.entity_id = e.id', array());
	...
	public function filterByExcludedTags($select, $tags)
	{
		...
		$subSelect->from(array('i'=>$db->Item), 'i.id')
		            ...
		            ->joinInner(array('t'=>$db->Tag), 'tg.tag_id = t.id', array());
	...
	public function getSelect($type='full')
	{
		...
		switch ($type) {
			case 'full':
				...
				$select->from( array('i'=>$db->Item),array('i.*', 'added'=>'added.time', 'modified'=>'modded.time'));
			
				//Join on the entities_relations table so we can pull timestamps
				$select->joinLeft( array('modded'=>"$db->EntitiesRelations"), 'modded.relation_id = i.id AND modded.type = "Item"', array());
				$select->joinLeft( array('mod_r'=>"$db->EntityRelationships"), 'mod_r.id = modded.relationship_id AND mod_r.name = "modified"', array());
	
				$select->joinLeft( array('added'=>"$db->EntitiesRelations"), 'added.relation_id = i.id AND added.type = "Item"', array());
				$select->joinLeft( array('add_r'=>"$db->EntityRelationships"), 'add_r.id = added.relationship_id AND add_r.name = "added"', array());
				break;
			
			//'Simple' SQL statement just returns id, title
			case 'simple':	
				$select->from( array('i'=>"$db->Item"), array('i.id','i.title'));
				break;
			default:
				# code...
				break;

    ...
    <?php if ($plugin->isInstalled()): ?>
        <?php if ($plugin->hasNewVersion()): ?>
            <?php if (has_permission('Plugins', 'upgrade')): ?>
                <form action="<?php echo html_escape(uri('plugins/upgrade')); ?>" method="post" accept-charset="utf-8">     
                    <div>
                        <button name="upgrade" type="submit" class="upgrade" value="<?php echo html_escape($plugin->getDirectoryName()); ?>"<?php if (!$loader->hasPluginBootstrap($plugin)): ?> disabled="disabled"<?php endif; ?>>Upgrade</button>
        ...
        <?php else: ?>
            ...
            <?php if (has_permission('Plugins', 'activate')): ?>
                <form action="<?php echo html_escape(uri('plugins/' . $activateOrDeactivate)); ?>" method="post" accept-charset="utf-8">
                    <div>
                        <button name="<?php echo $activateOrDeactivate; ?>" type="submit" class="<?php echo $activateOrDeactivate; ?>" value="<?php echo html_escape($plugin->getDirectoryName()); ?>"<?php if (!$loader->hasPluginBootstrap($plugin)): ?> disabled="disabled"<?php endif; ?>><?php echo ($plugin->isActive()) ? 'Deactivate' : 'Activate'; ?></button>
                        <input type="hidden" name="name" value="<?php echo html_escape($plugin->getDirectoryName()); ?>" />
                    </div>
                </form>                
            <?php endif; ?>
        ...
        <?php if (has_permission('Plugins', 'uninstall')): ?>
            <form action="<?php echo html_escape(uri(array(
                ...
                <div>
                    <button name="uninstall" type="submit" class="uninstall" value="<?php echo html_escape($plugin->getDirectoryName()); ?>"<?php if (!$loader->hasPluginBootstrap($plugin)): ?> disabled="disabled"<?php endif; ?>>Uninstall</button>
                    <input type="hidden" name="name" value="<?php echo html_escape($plugin->getDirectoryName()); ?>" />
                </div>
            </form>                
        <?php endif; ?>     
<?php else: //The plugin has not been installed yet ?>
    <?php if (has_permission('Plugins', 'install')): ?>
        <form action="<?php echo html_escape(uri('plugins/install')); ?>" method="post" accept-charset="utf-8">
                <div>
                    <button name="install" type="submit" class="install" value="<?php echo html_escape($plugin->getDirectoryName()); ?>"<?php if (!$loader->hasPluginBootstrap($plugin)): ?> disabled="disabled"<?php endif; ?>>Install</button>
                    <input type="hidden" name="name" value="<?php echo html_escape( $plugin->getDirectoryName()); ?>" />
                </div>
        </form> 
    <?php endif; ?>
